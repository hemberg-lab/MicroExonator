...
if str2bool(config.get("sashimi_tsv", False)):
	...
	rule ggsashmi_bulk_scripts:
		input:
			node = "Whippet/ggsashimi/{compare_name}/{ME}.txt",
			tsv = "Whippet/ggsashimi/{compare_name}/{compare_name}.tvs",
			gtf = config["Gene_anontation_GTF"],
			bams = lambda w: expand("Whippet/BAM/Merge/{cluster}.sort.bam",  cluster=compare_all_clusters[w.compare_name])
        params:
			...
			out = "Whippet/ggsashimi/{compare_name}/{gene}_{node}_{strand}"
		output:
			"ggsashimi/{gene}_{node}_{strand}.sh"
			#"Whippet/ggsashimi/{compare_name}/{gene}_{node}_{strand}.pdf"
		shell:
			"python src/sashimi-plot.py -b {input.tsv} -c {params.region} -g {input.gtf} -o {params.out} > {output} "

	rule get_sashimis_bulk:
		input:
			expand("ggsashimi/{sashimi}.pdf", sashimi=ashimis_bulk)
