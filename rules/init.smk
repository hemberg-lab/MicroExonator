    ...
    with open("NCBI_accession_list.txt") as file :
        ...
        for row in reader:
            ...
            if len(glob.glob(file_name))==0: #Check if the file is there, as if this file is overwriten everything will start from scratch
                ...
                # Mates get _1 and _2 name suffixes, so the two reads of a fragment stay distinct.
                download_file.write( "if [ $numLines -eq 8 ]; then bash src/concat_mates.sh FASTQ/${srr}_1.fastq.gz FASTQ/${srr}_2.fastq.gz FASTQ/$srr.fastq.gz && rm FASTQ/${srr}_1.fastq.gz FASTQ/${srr}_2.fastq.gz; fi"  + "\n")
