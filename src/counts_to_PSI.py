...
def main(total_cov, min_sum_PSI, paired, out_PSI):
  ...
  with  gzip.open(out_PSI, "wt") as out:
    ...
    out.write( "\t".join(["File", "ME_coords", "SJ_coords", "ME_coverages", "SJ_coverages", "PSI", "CI_Lo", "CI_Hi", "Alt5", "Alt3", "Alt5_coverages", "Alt3_coverages", "Unique_ME_reads", "sum_ME_SJ_coverage_up", "sum_ME_SJ_coverage_down", "Shared_site_half_reads"])+ "\n")    
    ...
    with open(total_cov) as file:
      ...
      for line in reader:
        ...
        shared_site_half = row.get('shared_site_half', 'NA')
        ...
        if row["FILE_NAME"] in paired_files:
          ...
          info = [Unique_ME_reads, sum_ME_SJ_coverage_up, sum_ME_SJ_coverage_down, sum_ME_coverage, sum_SJ_coverage, total_cov_alternatives_3, total_cov_alternatives_5, row["FILE_NAME"], row["ME"], row["total_SJs"], row["ME_SJ_coverages"], row["SJ_coverages"], row["alternatives_5"], row["alternatives_3"], row["cov_alternatives_5"], row["cov_alternatives_3"], shared_site_half]
          paired_info[(row["FILE_NAME"], row["ME"])] = info

        else:

          out.write( "\t".join(map(str,  [row["FILE_NAME"], row["ME"], row["total_SJs"], row["ME_SJ_coverages"], row["SJ_coverages"], PSI, CI_Lo, CI_Hi, row["alternatives_5"], row["alternatives_3"], row["cov_alternatives_5"], row["cov_alternatives_3"], row["sum_ME_SJ_coverage_up_down_uniq"], row["sum_ME_SJ_coverage_up"], row["sum_ME_SJ_coverage_down"], shared_site_half]) )+ "\n")	
      ...
      for p, p1info in paired_info.items():
        ...
        if FILE_NAME_1 in pair12:
          ...
          Unique_ME_reads_1, sum_ME_SJ_coverage_up_1, sum_ME_SJ_coverage_down_1, sum_ME_coverage_1, sum_SJ_coverage_1, total_cov_alternatives_3_1, total_cov_alternatives_5_1, FILE_NAME, ME, total_SJs, ME_SJ_coverages, SJ_coverages, alternatives_5, alternatives_3, cov_alternatives_5, cov_alternatives_3, shared_site_half_1 = p1info
          ...
          shared_site_half_2 = p2info[-1]
          shared_site_half = "NA" if "NA" in (shared_site_half_1, shared_site_half_2) else float(shared_site_half_1) + float(shared_site_half_2)
          ...
          out.write( "\t".join(map(str,  [FILE_NAME, ME, total_SJs, ME_SJ_coverages, SJ_coverages, PSI, CI_Lo, CI_Hi, alternatives_5, alternatives_3, cov_alternatives_5, cov_alternatives_3,  Unique_ME_reads, sum_ME_SJ_coverage_up, sum_ME_SJ_coverage_down, shared_site_half] ) )+ "\n")	
