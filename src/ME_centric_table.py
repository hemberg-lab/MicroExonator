...
###Cheking if any ME is in two or more SJ
    ...
    for i in list(SJ_SJ.items()):

        SJ_same_ME.add(" ".join(sorted(list(i[1]))))

####Coverage dict #####

    for i in list(ME_reads.items()):
...
####
    ...
    for i in SJ_same_ME:
        ...
        if total_ME!="":   #The empty fields refeclts no interesection between micro-exons present on the splice junctions
            ...
            out =  list(map(str, [ME, transcript, sum_total_coverage, total_SJs, total_coverages, len_micro_exon_seq_found, micro_exon_seq_found, total_number_of_micro_exons_matches, U2_scores, mean_conservations, min(P_MEs), total_ME]))
