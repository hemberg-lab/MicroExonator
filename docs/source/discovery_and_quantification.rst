...
* ``consecutive_microexons`` (default ``T``) corrects counts for microexons that sit next to another microexon (for example E1-m1-m2-E2). The junctions that skip only one of them (E1|m2 skips m1) are the neighbour's inclusion junctions and have no skipping tag, and a read on the shared m1|m2 junction fits both microexons' inclusion tags but is aligned to only one. With this option, a read that crosses a junction of an inclusion tag (at least 8 nt on each side) also counts as exclusion for every other microexon inside that junction, and as inclusion for an adjacent microexon on the other side of that junction. Only the immediate junction is used, so runs of three or more microexons are handled too. Junctions added this way appear in ``SJ_coords``. Set it to ``F`` to reproduce results from earlier versions.
