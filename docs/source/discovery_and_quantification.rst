...
* ``max_read_len`` (default ``100``) is the length of the exon sequence on each side of every splice-junction tag. Set it to the read length of your longest reads. Reads are aligned end to end, so a read longer than a tag flank only aligns where it fits inside the tag; this used to over-count microexon inclusion (more so for longer microexons) and discard most junction reads. Reads longer than ``max_read_len`` are now trimmed to that length before they are aligned to the tags, so results are unbiased either way, but setting it to the read length keeps more reads.
