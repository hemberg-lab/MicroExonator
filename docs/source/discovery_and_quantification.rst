...
* ``max_read_len`` (default ``150``) is the length of exon sequence on each side of every splice-junction tag, and the length reads are trimmed to before they are aligned to the tags. Reads are aligned end to end, so a read longer than the tag flanks only aligns where it happens to fit inside a tag; inclusion tags are longer than skipping tags by the microexon's length, so such reads over-count inclusion (by up to 60% for a 30 nt microexon with 150 nt reads on 100 nt flanks) and most junction reads are lost. Any read up to ``max_read_len`` + 8 nt fits in every position where it crosses a junction with the minimum 8 nt anchors, so one tag library serves all shorter read lengths without bias. The default of 150 covers the common bulk formats (for example 2x150 and anything shorter); longer reads are trimmed to 150. Raising it keeps more of very long reads at the cost of a larger tag library; lowering it only reduces the library size. ``Report/read_lengths.tsv`` lists, per sample, the number of reads, their median and maximum length, and the percentage trimmed, and notes samples with a median read shorter than 40 nt or with most reads trimmed, which often means another library type (for example single-cell data) was included by mistake.
...
* ``consecutive_microexons`` (default ``T``) corrects counts for microexons that sit next to another microexon (for example E1-m1-m2-E2). The junctions that skip only one of them (E1|m2 skips m1) are the neighbour's inclusion junctions and have no skipping tag, and a read on the shared m1|m2 junction fits both microexons' inclusion tags but is aligned to only one. With this option, a read that crosses a junction of an inclusion tag (at least 8 nt on each side) also counts as exclusion for every other microexon inside that junction, and as inclusion for an adjacent microexon on the other side of that junction. Only the immediate junction is used, so runs of three or more microexons are handled too. Junctions added this way appear in ``SJ_coords``. Microexons in runs are listed in ``Report/ME_consecutive_runs.txt``; in runs of three or more, reads that reach a microexon two positions away are not recovered, so PSI can be slightly underestimated (by up to about 0.05 in simulations). Set it to ``F`` to reproduce results from earlier versions.

* ``mismatch_aware_blacklist`` (default ``T``) changes how reads that also align to the genome without splicing are removed. By default a read is removed only if its genome alignment has no more mismatches than its splice-junction tag alignment, so reads that fit a junction better than a retrocopy (processed pseudogene) of it are kept. Set it to ``F`` to remove every read with a genome alignment, as in earlier versions. Reads that fit a junction and its genomic copy equally well are still removed, so junctions with an unspliced copy of up to 2 mismatches in the genome are listed in ``Report/ME_junction_genomic_copies.txt`` with the fewest mismatches of any copy (a copy of the skipping junction biases PSI upwards, a copy of the inclusion junction downwards). Very short microexons can fit several positions with canonical splice sites in the same intron, which reads cannot tell apart; MicroExonator reports the position with the best splice-site score and lists the alternatives in ``Report/ME_ambiguous_positions.txt``.
